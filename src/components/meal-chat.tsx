...
export default function MealChat({ onClose }: ChatBotProps) {
    const [messages, setMessages] = useState<Message[]>([
        {
            id: "1",
            text: "Hi! I'm your meal plan assistant. Tell me your budget, days, and preferences.",
            sender: "bot",
            timestamp: new Date(),
        },
    ])
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            doc.text("Meal Plan", pageWidth / 2, yPosition, { align: "center" })
            yPosition += 15

            // Summary
            doc.setFontSize(12)
            doc.setTextColor(0, 0, 0)
            doc.text(`Total Budget: $${mealPlan.total_budget}`, 20, yPosition)
            yPosition += 7
            doc.text(`Days: ${mealPlan.days}`, 20, yPosition)
            yPosition += 7
            doc.text(`Total Cost: $${mealPlan.total_cost}`, 20, yPosition)
            yPosition += 7
            doc.text(`Savings: $${mealPlan.remaining_budget}`, 20, yPosition)
            yPosition += 15
            ...
            mealPlan.daily_plans?.forEach((dailyPlan) => {
                ...
                doc.text(`Day ${dailyPlan.day} - Total: $${dailyPlan.daily_total}`, 20, yPosition)
                yPosition += 10
            ...
            // Save PDF
            doc.save(`meal-plan-${new Date().toLocaleDateString()}.pdf`)
        } catch (error) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        const warningMessage: Message = {
            ...
            text: "⏳ Generating your meal plan... This may take a moment, please wait.",
            sender: "bot",
            timestamp: new Date(),
        }
                ...
                <div className="flex h-full flex-col bg-white">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-white">{showResults ? "Your Meal Plan" : "Meal Assistant"}</h2>
                                <p className="text-xs text-white/80">{showResults ? "Personalized plan" : "Online"}</p>
                            </div>
                        </div>
                            ...
                            <div className="border-t border-gray-200 px-4 py-4">
                                <form onSubmit={handleSendMessage} className="flex items-end gap-2">
                                    <div className="flex-1">
                                        <Input
                                            type="text"
                                            placeholder="Ex: Budget 500, 2 days, healthy food..."
                                            value={inputValue}
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    {mealPlan.daily_plans && mealPlan.daily_plans.length > 0 ? (
                                        ...
                                        <div className="text-center py-8 text-gray-500">
                                            <p>No meal plans found in response.</p>
                                        </div>
                                    )}
                                    ...
                                    <div className="sticky bottom-0 bg-white border-t-4 border-[var(--brand-blue)] rounded-t-2xl shadow-2xl p-6 -mx-6 -mb-6 animate-in slide-in-from-bottom duration-700">
                                        <div className="flex items-center justify-between mb-4">
                                            <div>
                                                <p className="text-sm text-gray-600 mb-1">Plan Total</p>
                                                <p className="text-3xl font-bold text-[var(--brand-blue)]">${mealPlan.total_cost}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm text-gray-600 mb-1">You saved</p>
